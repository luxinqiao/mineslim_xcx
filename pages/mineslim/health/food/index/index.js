...
Page({
	...
	initData(){
		...
		md_request.api({
			path: md_request.config.yy + '/wechat/health/food',
			data: dataObj,
			callback: res => {
                this.setData({
                    isReady: true
				...
				if (res.data.code == '200') {
                    ...
                    if(!md_common.isEmpty(data.pro)) {
                        if(!md_common.isEmpty(data.pro.target)) {
                            data.pro.target.map((item)=>{
                    ...
                    if(md_common.isEmpty(data.family)&&!md_common.isEmpty(data.pro)) {
                        this.setData({
                            tabName: 'major'
                        })
                    }
	 ...
	 */
	introdMajorUse() {
		this.selectComponent('#mdPopupMajor').showPopup()
    },
    ...
    clock(e) {
        ...
        md_request.api({
            path: md_request.config.yy + '/wechat/health/food-mark',
            data: {
                token: wx.getStorageSync('token'),
                record_id: this.data.dataObj.record_id,
                type: type
            },
            callback: res => {
                if (res.data.code == '200') {
                    this.selectComponent('#mdDialog').alert('饮食打卡已成功！','确认',()=>{
                        this.initData()
                    })
    ...
    onShareAppMessage() {
        ...
        if(this.data.tabName == 'home') {
            src =  this.selectComponent('#mdPosterFam').getImgSrc()
            path = `/pages/mineslim/health/food/index/index?id=${this.data.dataObj.record_id}&tabName=home&isShare=true`
        } else {
            src =  this.selectComponent('#mdPosterPro').getImgSrc()
            path = `/pages/mineslim/health/food/index/index?id=${this.data.dataObj.record_id}&tabName=major&isShare=true`
        }
        return {
			title: '吃起来要个性',
			imageUrl: src,
